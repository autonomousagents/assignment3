 ...
 */
     ...
     */
    public StateRepV (double init, boolean isPrey ){
        stateRep = new double[stateRepHeight][stateRepWidth];
        for(double[] row: stateRep)Arrays.fill(row, init);
        fillUnused();
        this.isPrey = isPrey;
    }
    
    public double getV(int linearIndex){
    ...
    public void printAll (boolean latex){
        if(latex){ 
            for(int i =0;i<stateRepHeight;i++){
                for(int j = 0; j<stateRepWidth;j++){
                    if(stateRep[i][j]!=absentValue){
                        System.out.print(String.format("%.4f",stateRep[i][j]));
                    }
                    if(j==stateRepWidth-1){
                    System.out.print("\\\\ \n");
                    }
